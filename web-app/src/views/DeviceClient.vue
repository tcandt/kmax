  ...
  <div class="device-panel-view" :class="{ 'is-mobile': isMobile, 'mobile-landscape': isMobile && isVideoLandscape, 'is-web-fullscreen': isWebFullscreen, 'is-camera-surveillance': isCameraMode }" @mousemove="onFsUiActivity" @touchstart.passive="onFsUiActivity">
    ...
    <div class="device-client-main">
      ...
      <div class="video-wrapper" ref="containerRef">
        <!-- Camera surveillance OSD watermark bar -->
        <div v-if="isCameraMode" class="camera-osd-bar">
          <div class="osd-left">
            <span class="osd-badge live-dot">● Live Surveillance</span>
            ...
            <span class="osd-item osd-bitrate" v-if="videoStats" :title="`Video Bitrate (Target: ${videoStats.targetBitrate || localSettings.bitrate || 4} Mbps)`">
              {{ videoStats.bitrate > 1000 ? (videoStats.bitrate / 1000).toFixed(1) + ' Mbps' : videoStats.bitrate + ' kbps' }}
              <span v-if="isWebSocketMode" class="osd-sub"> (Target {{ videoStats.targetBitrate || localSettings.bitrate || 4 }}M)</span>
            </span>
          </div>
        ...
        <!-- Mobile exit button -->
        <button v-if="isMobile" class="mobile-close-fab" @click="deviceStore.clearActiveDevice()" title="Close connection">
          ✕
        </button>

        <!-- Floating fullscreen button -->
        <button class="fullscreen-fab" @click="toggleFullscreen" title="System Fullscreen">
        ...
        <button 
          ...
          :title="isWebFullscreen ? 'Exit Web Fullscreen (Esc)' : 'Web Fullscreen'"
        >
        ...
        <button v-if="!isMobile && pictureInPictureSupported" class="pip-fab" @click="togglePictureInPicture" :title="isPiP ? 'Exit Picture-in-Picture' : 'Picture-in-Picture'">
        ...
        <!-- WebCodecs / WebSocket Canvas Renderer -->
        <canvas
        ...
        <div v-if="videoStats && localSettings.showStats !== false && !isCameraMode" class="stats-badge">
          <span v-if="isWebSocketMode" class="stat-conn-type ws-pill" title="WebSocket TCP Binary Stream">⚡ WS Stream</span>
          <span class="stat-fps">{{ videoStats.fps }}fps</span>
          <span class="stat-delimiter">|</span>
          <template v-if="!isWebSocketMode">
            <span class="stat-delay" title="Network RTT + JB Buffer + Decode + Cloud Render">E2E ~{{ videoStats.e2eDelay }}ms</span>
            <span class="stat-delimiter">|</span>
            <span class="stat-delay" title="Jitter Buffer">JB {{ videoStats.jbDelay }}ms</span>
            <span class="stat-delimiter">|</span>
            <span class="stat-delay" title="Round Trip Time (RTT)">RTT {{ videoStats.rtt }}ms</span>
            <span class="stat-delimiter">|</span>
          </template>
          <span class="stat-bitrate" :title="`Video Bitrate (Target: ${videoStats.targetBitrate || localSettings.bitrate || 4} Mbps)`">
            {{ videoStats.bitrate > 1000 ? (videoStats.bitrate / 1000).toFixed(1) + ' Mbps' : videoStats.bitrate + ' kbps' }}
            <span v-if="isWebSocketMode" class="stat-sub"> (Target {{ videoStats.targetBitrate || localSettings.bitrate || 4 }}M)</span>
          </span>
          <span class="stat-delimiter">|</span>
          <span class="stat-conn-type" :title="isWebSocketMode ? 'WebSocket TCP Pure Stream' : 'WebRTC Channel Type'">{{ isWebSocketMode ? 'TCP (WebSocket)' : (videoStats.connectionType || 'UDP p2p') }}</span>
          <template v-if="!isWebSocketMode">
            <span class="stat-delimiter">|</span>
        ...
        <div v-if="showOverlay" class="panel-overlay">
          <div class="overlay-box">
            ...
            <template v-else-if="currentWebRTC.error.value">
              <p class="error-msg">❌ Connection Failed</p>
              <p class="error-tip">{{ currentWebRTC.error.value }}</p>
              <button class="retry-btn" @click="retry">Retry</button>
              <template v-if="!isWebSocketMode">
                <p class="error-tip ws-fallback-hint">If WebRTC repeatedly fails due to UDP blocked by firewall/NAT, try TCP tunnel:</p>
                <button class="retry-btn ws-fallback-btn" @click="toggleStreamMode">⚡ Switch to WebSocket Stream</button>
              </template>
            </template>
            <template v-else-if="currentWebRTC.status.value === 'disconnected'">
              <p>Disconnected</p>
              <button class="retry-btn" @click="retry">Reconnect</button>
            </template>
          </div>
        </div>
        ...
        <div v-if="isMobile || isFullscreen || isWebFullscreen" class="mobile-fab-container" :style="fabStyle">
          ...
          <div class="mobile-fab-menu" :class="{ 'show': showMobileMenu, 'align-left': isFabOnLeft, 'align-top': isFabOnTop }">
            <!-- Surveillance mode shortcuts -->
            <template v-if="isCameraMode">
              <div class="fab-section-title">Lens Selection</div>
              <button 
              ...
              <div class="fab-section-title">Hardware Resolution</div>
              ...
              <button class="fab-item" @click="rotateCamera(); showMobileMenu=false">
                🔄 Rotate 90° (Current {{ cameraRotation }}°)
              </button>
              ...
              <button class="fab-item" :class="{ 'recording': isRecording }" @click="toggleCameraRecording(); showMobileMenu=false">
                ...
                🔴 {{ isRecording ? `Stop Recording (${formattedRecordingTime})` : 'Instant Recording' }}
              </button>
              <button class="fab-item" @click="togglePageMute(); showMobileMenu=false">
                {{ pageAudioMuted ? '🔊 Unmute Audio' : '🔇 Mute Audio' }}
              </button>
              <div class="fab-divider"></div>
              <button class="fab-item" @click="showSettingsModal = true; showMobileMenu=false">
                <svg class="icon" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg> Camera Settings
              </button>
              <button class="fab-item danger" @click="goBackToList(); showMobileMenu=false">
                <svg class="icon" viewBox="0 0 24 24"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg> Disconnect Camera
              </button>
            </template>

            <!-- Standard cloud phone floating menu -->
            <template v-else>
              <button v-if="authStore.isAdmin" class="fab-item" :class="{ 'group-active': groupControlStore.isGroupControlActive }" @click="toggleGroupControl(); showMobileMenu=false">
                ...
                {{ groupControlStore.isGroupControlActive ? 'Exit Master' : 'Group Master' }}
              </button>
              <button class="fab-item" :class="{ 'group-active': isWebSocketMode }" @click="toggleStreamMode(); showMobileMenu=false">
                ...
                {{ isWebSocketMode ? 'Switch to WebRTC' : 'Switch to WebSocket' }}
              </button>
              <div class="fab-divider"></div>
              ...
              <button class="fab-item" @click="quickKey('input keyevent 25'); showMobileMenu=false">
                <svg class="icon" viewBox="0 0 24 24"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="19" y1="12" x2="15" y2="12"></line></svg> Vol-
              </button>
              <button class="fab-item" @click="togglePageMute(); showMobileMenu=false">
                ...
                {{ pageAudioMuted ? 'Unmute' : 'Mute' }}
              </button>
              <button class="fab-item" @click="toggleConsole(); showMobileMenu=false">
                <svg class="icon" viewBox="0 0 24 24"><polyline points="4 17 10 11 4 5"></polyline><line x1="12" y1="19" x2="20" y2="19"></line></svg> Terminal
              </button>
              <button class="fab-item" @click="keymapStore.setEditMode(true); showMobileMenu=false">
                <svg class="icon" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="3"></circle></svg> Keymap
              </button>
              <button class="fab-item" @click="keymapStore.toggleKeyHints(); showMobileMenu=false">
                ...
                {{ keymapStore.showKeyHints ? 'Hide Hints' : 'Show Hints' }}
              </button>
              <button class="fab-item" @click="showSettingsModal = true; showMobileMenu=false">
              ...
              <button class="fab-item" @click="sendClipboardToDevice(); showMobileMenu=false">
                <svg class="icon" viewBox="0 0 24 24"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"></path><rect x="8" y="2" width="8" height="4" rx="1" ry="1"></rect></svg> Send Clip
              </button>
              ...
              <div v-for="(btn, idx) in customButtons" :key="idx" class="fab-item-wrapper">
                ...
                <button class="fab-item-delete" @click.stop="removeCustomButton(idx)" title="Delete this key">×</button>
              </div>
              <button class="fab-item add-btn" @click="addCustomButton">
                <svg class="icon" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg> Add Key
              </button>
              
              <div class="fab-divider"></div>
              <button class="fab-item danger" @click="goBackToList(); showMobileMenu=false">
                <svg class="icon" viewBox="0 0 24 24"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg> Disconnect
              </button>
              <button class="fab-item danger" @click="quitAgent(); showMobileMenu=false">
                <svg class="icon" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="9" y1="9" x2="15" y2="15"></line><line x1="15" y1="9" x2="9" y2="15"></line></svg> Quit Agent
              </button>
            </template>
            <div class="fab-agent-version" :title="'Agent Version: ' + agentVersion">
    ...
    <!-- PC Right sidebar (Standard Mode) -->
    <div v-if="!isMobile && !isMini && !isCameraMode" class="control-sidebar" :class="{ 'fs-ui-visible': fsUiVisible }">
      <div class="sidebar-group">
        <button v-if="authStore.isAdmin" class="sidebar-btn group-control-btn" :class="{ active: groupControlStore.isGroupControlActive }" @click="toggleGroupControl" :title="groupControlStore.isGroupControlActive ? 'Exit group master mode' : 'Set as group master'">
          ...
          <span class="btn-text">{{ groupControlStore.isGroupControlActive ? 'Exit Master' : 'Group Master' }}</span>
        </button>
        <button class="sidebar-btn" :class="{ active: isWebSocketMode }" @click="toggleStreamMode" :title="isWebSocketMode ? 'WebSocket active, click to switch to WebRTC' : 'WebRTC active, click to switch to WebSocket'">
          <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon>
          </svg>
          <span class="btn-text">{{ isWebSocketMode ? 'WS Stream' : 'WebRTC' }}</span>
        </button>
        <div class="sidebar-divider"></div>
        <button class="sidebar-btn" @click="quickKey('input keyevent 26')" title="Power">
        ...
        <button class="sidebar-btn" @click="quickKey('input keyevent 24')" title="Volume Up">
          ...
          <span class="btn-text">Vol+</span>
        </button>
        <button class="sidebar-btn" @click="quickKey('input keyevent 25')" title="Volume Down">
        ...
        <button class="sidebar-btn" :class="{ active: pageAudioMuted }" @click="togglePageMute" :title="pageAudioMuted ? 'Unmute' : 'Mute'">
          <svg v-if="pageAudioMuted" class="icon" viewBox="0 0 24 24"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="23" y1="9" x2="17" y2="15"></line><line x1="17" y1="9" x2="23" y2="15"></line></svg>
          <svg v-else class="icon" viewBox="0 0 24 24"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M15 9a5 5 0 0 1 0 6"></path><path d="M17.7 6.3a9 9 0 0 1 0 11.4"></path></svg>
          <span class="btn-text">{{ pageAudioMuted ? 'Unmute' : 'Mute' }}</span>
        </button>
        <button class="sidebar-btn" :class="{ active: deviceStore.showGlobalConsole && deviceStore.consoleDeviceId === currentId }" @click="toggleConsole" title="Console">
        ...
        <button class="sidebar-btn" @click="keymapStore.setEditMode(true)" title="Keymap">
          <svg class="icon" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="3"></circle></svg>
          <span class="btn-text">Keymap</span>
        </button>
        <button class="sidebar-btn" @click="keymapStore.toggleKeyHints()" :title="keymapStore.showKeyHints ? 'Hide key hints' : 'Show key hints'">
          <svg v-if="keymapStore.showKeyHints" class="icon" viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
          <svg v-else class="icon" viewBox="0 0 24 24"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>
          <span class="btn-text">Hints</span>
        </button>
        <button class="sidebar-btn" @click="sendClipboardToDevice" title="Send clipboard to device">
          ...
          <span class="btn-text">Send Clip</span>
        </button>
        <button class="sidebar-btn" @click="getClipboardFromDevice" title="Get clipboard from device">
          <svg class="icon" viewBox="0 0 24 24">
            <path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"></path>
            <rect x="8" y="2" width="8" height="4" rx="1" ry="1"></rect>
            <path d="M12 11v6M9 14l3 3 3-3" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none"></path>
          </svg>
          <span class="btn-text">Get Clip</span>
        </button>
      </div>
      ...
      <div class="sidebar-group custom-group">
        <div v-for="(btn, idx) in customButtons" :key="idx" class="sidebar-btn-wrapper">
          ...
          <button class="sidebar-btn-delete" @click.stop="removeCustomButton(idx)" title="Delete this key">×</button>
        </div>
        <button class="sidebar-btn add-btn" @click="addCustomButton" title="Add key">
      ...
      <button class="sidebar-btn danger" @click="quitAgent" title="Quit Agent">
      ...
      <button class="sidebar-btn" @click="showSettingsModal = true" title="Connection settings">
        <svg class="icon" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg>
        <span class="btn-text">Settings</span>
      </button>

      <!-- Agent version footer -->
      <div class="sidebar-agent-version" :title="'Agent Version: ' + agentVersion">
        {{ agentVersion.split('-')[0] }}
      </div>
    </div>

    <!-- PC Right sidebar (Surveillance Mode) -->
      ...
      <div class="camera-sidebar-scroll">
        ...
        <div class="cam-section">
          ...
          <div class="cam-btn-grid zoom-presets">
            ...
            <button class="cam-btn reset-btn" @click="resetPTZ" title="Reset zoom and pan">Reset</button>
          </div>
        </div>
        ...
        <div class="cam-section">
          <div class="cam-section-header">
            ...
            <span class="cam-badge">{{ cameraRotation }}° {{ cameraMirrored ? 'Mirror' : '' }}</span>
          </div>
          <div class="cam-btn-grid">
            <button class="cam-btn" @click="rotateCamera" title="Rotate 90 degrees clockwise">
            ...
            <button class="cam-btn" :class="{ active: cameraMirrored }" @click="toggleMirror" title="Horizontal mirror flip">
        ...
        <div class="cam-section">
          <div class="cam-section-header">
            ...
            <span class="cam-badge" :class="{ 'audio-on': !pageAudioMuted }">{{ pageAudioMuted ? 'Muted' : 'Listening' }}</span>
          </div>
          <div class="audio-control-row">
            <button class="cam-btn audio-toggle-btn" :class="{ active: !pageAudioMuted }" @click="togglePageMute">
              ...
              <span>{{ pageAudioMuted ? 'Unmute Audio' : 'Mute Audio' }}</span>
            </button>
          </div>
        </div>
        ...
        <div class="cam-section">
          ...
          <div class="cam-btn-grid">
            <button class="cam-btn snapshot-btn" @click="takeCameraSnapshot" title="Capture HD snapshot as JPG">
            ...
            <button 
              ...
              :title="isRecording ? 'Stop recording and download' : 'Start local recording'"
            >
              <span class="rec-dot"></span>
              <span>{{ isRecording ? `Stop (${formattedRecordingTime})` : 'Local Record' }}</span>
            </button>
          </div>
        </div>
        ...
        <div class="cam-section bottom-section">
          <div class="cam-btn-grid">
            <button class="cam-btn" @click="showSettingsModal = true" title="Advanced camera & codec settings">
              <svg class="icon" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg>
              <span>Settings</span>
            </button>
            <button class="cam-btn danger-btn" @click="goBackToList" title="Exit camera and return to list">
...
// Multi-device focus and audio detection
...
// --- Demo mode simulation Canvas driver ---
...
// --- Group control slave render and event dispatch ---
...
// Group control broadcast dispatch (with scroll throttle)
...
// --- 📷 Camera surveillance state and control ---
...
const availableLenses = computed(() => {
  ...
  if (Array.isArray(detected) && detected.length > 0) {
    return detected.map(cam => {
      let icon = '📷'
      let shortName = cam.name || `Lens ${cam.id}`
      ...
      if (cam.type === 'ultra_wide' || (cam.focal_length && cam.focal_length < 2.8) || zoom < 0.9) {
        icon = '🌐'
        shortName = zoom <= 0.6 ? `${zoom.toFixed(1)}x Wide` : 'Ultra Wide'
      } else if (cam.type === 'telephoto' || (cam.focal_length && cam.focal_length >= 7.0) || zoom >= 2.5) {
        icon = '🔭'
        shortName = `${zoom.toFixed(1)}x Tele`
  ...
  return [
    { key: 'back_main', id: '0', facing: 'back', zoom: 1.0, name: 'Rear Main (1.0x)', shortName: '1.0x Main', icon: '📷' },
    { key: 'back_wide', id: '0', facing: 'back', zoom: 0.5, name: 'Rear Ultra Wide (0.5x)', shortName: '0.5x Wide', icon: '🌐' },
    { key: 'back_tele', id: '0', facing: 'back', zoom: 5.0, name: 'Rear Telephoto', shortName: 'Telephoto', icon: '🔭' },
    { key: 'front_1', id: '1', facing: 'front', zoom: 1.0, name: 'Front Camera', shortName: 'Front', icon: '🤳' },
    { key: 'external', id: '', facing: 'external', zoom: 1.0, name: 'External Camera', shortName: 'External', icon: '🔌' }
  ]
})
...
const currentLensName = computed(() => {
  ...
  if (localSettings.value.cameraFacing === 'front') return 'Front'
  if (localSettings.value.cameraFacing === 'external') return 'External'
  return 'Rear Main'
})
...
function selectLens(lens) {
  ...
  reconnectStream(`Switching to [${lens.shortName || lens.name}]...`)
}
...
function selectResolution(res) {
  ...
  reconnectStream(`Switching resolution to [${label}]...`)
}

function reconnectStream(msg = 'Reconnecting with updated parameters...') { 
...
function takeCameraSnapshot() {
  ...
  a.download = `snapshot_${currentId.value}_${timeStr}.jpg`
  a.href = dataUrl
  a.click()
}
...
function startRecording() {
  ...
  if (!stream) {
    alert('Video stream is not ready, cannot start recording')
    return
  }
  recordedChunks = []
  try {
    ...
    mediaRecorder.onstop = () => {
      ...
      const pad = (n) => String(n).padStart(2, '0')
      const timeStr = `${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}`
      a.download = `recording_${currentId.value}_${timeStr}.${mimeType.includes('mp4') ? 'mp4' : 'webm'}`
      a.href = url
      a.click()
    ...
    alert('Cannot start local recording: ' + err.message)
  }
}
...
function saveSettings(newSettings) {
  ...
  // WebSocket direct control: dynamic params update
  ...
  // WebRTC mode reconnect helper
  // Wait for Agent restart
...
// Mobile floating menu state and drag
...
function addCustomButton() {
  const name = prompt('Button name (e.g. Action):')
  if (!name) return
  const cmd = prompt('ADB Shell command (e.g. settings put system pointer_location 1):')
  if (!cmd) return
...
function removeCustomButton(index) {
  if (confirm('Are you sure you want to delete this custom button?')) {
...
function toggleStreamMode() {
  ...
  if (webrtc && typeof webrtc.disconnect === 'function') {
    webrtc.disconnect()
  }
...
function onGlobalKeyDown(e) {
  // Multi-device focus isolation: non-focused devices ignore global keyboard
...
// Connection mode changes handled by MultiDeviceItem key remount
// Avoid duplicate reconnects
...
function setupWebRTC() {
  ...
  // Command results handled in DeviceConsole

  // Setup clipboard callback
  webrtc.onClipboard((event) => {
    // Focus isolation: clipboard
    if (!effectiveFocused.value) return
...
const handlePopState = (e) => {
  // Back button handler
  // Prevent default and disconnect
  deviceStore.clearActiveDevice()
}

function onWindowFocus() {
  if (isComposingText.value) return
  // Focus isolation: clipboard sync
  if (!effectiveFocused.value) return
...
onUnmounted(() => {
  ...
  // Note: do not reset connect mode on unmount
  ...
  if (navigator.clipboard && navigator.clipboard.readText) {
    navigator.clipboard.readText().then(text => {
      if (text) {
        ...
        alert('Local clipboard is empty')
      }
    }).catch(err => {
      const text = prompt('Enter text to send to device clipboard:', lastLocalClipboardText)
      if (text) {
        setDeviceClipboard(text, { paste: true, source: CLIPBOARD_SOURCE_LOCAL })
      }
    })
...
const loadingText = computed(() => {
  if (currentWebRTC.value.status.value === 'waiting_offer') return 'Waiting for device...'
  return 'Connecting...'
})

// Overlay: current user and account expiration
...
const connMetaText = computed(() => {
  ...
  if (ms <= 0) return `👤 ${name} · Account Expired`
  return `👤 ${name} · ${formatDuration(ms)} left`
})

const showOverlay = computed(() => currentWebRTC.value.status.value !== 'connected')

// Rotation coordinate check
const needRotateCoords = computed(() => isMobile.value && isVideoLandscape.value)

// Store video dimensions for coordinate translation
...
// Fallback to CSS web fullscreen
...
// Auto hide floating UI in fullscreen
...
function quitAgent() {
  if (confirm(`Warning: Stop Agent process on device "${currentId.value}"? The device will go offline.`)) {
...
.device-panel-view.is-web-fullscreen .webfullscreen-fab {
  position: absolute !important;
  top: 20px !important;
  right: 84px !important; /* Offset for right floating toolbar */
...
/* Group control slave sidebar layout */
...
/* =========================================================
   📷 Surveillance Mode Console Styles
   ========================================================= */

/* Camera OSD Watermark Bar */
.camera-osd-bar {
  position: absolute;
  top: 0;
  left: 0;
...
/* Mobile floating menu camera extensions */
.fab-section-title {
  font-size: 11px;
  font-weight: 700;
  color: #8b949e;
