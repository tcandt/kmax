...
export const useAuthStore = defineStore('auth', () => {
  ...
  // Current user policy (/api/me, null = not loaded):
  ...
  async function checkNoAuthStatus() {
    try {
      ...
      // Request failed, authentication required, ignore
    }
  }
  ...
  async function fetchMe() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // If server returns user AI config, save to localStorage
        if (data.ai_config) {
        ...
        // Cache user policy (quality/audio lock + admin settings + account expiration)
        userPolicy.value = {
