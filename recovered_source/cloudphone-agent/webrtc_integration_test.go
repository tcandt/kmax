...
func TestScrcpyOptions_RemoteCameraMode(t *testing.T) {
	...
	args := proc.buildArgs(clientOpts, 0x12345678, 0)
...
func TestH264_KeyframeWithSPSPrepend(t *testing.T) {
	streamer := NewStreamerBridge(nil, 30)

	// Realistic SPS (Profile 0x42, Constraint 0xe0, Level 0x1f) + PPS
	sps := []byte{0x00, 0x00, 0x00, 0x01, 0x67, 0x42, 0xe0, 0x1f, 0x8d, 0x68}
	pps := []byte{0x00, 0x00, 0x00, 0x01, 0x68, 0xce, 0x38, 0x80}
	configPayload := append(append([]byte{}, sps...), pps...)

	// 1. Simulate config frame arrival
	streamer.configMu.Lock()
	streamer.cachedCodecConfig = make([]byte, len(configPayload))
	copy(streamer.cachedCodecConfig, configPayload)
	streamer.configMu.Unlock()

	// 2. Simulate raw IDR frame without SPS (typical scrcpy keyframe)
	idrPayload := []byte{0x00, 0x00, 0x00, 0x01, 0x65, 0x88, 0x84, 0x00, 0x10, 0x20, 0x30, 0x40}
	if hasSPS(idrPayload) {
		t.Fatal("hasSPS should return false for pure IDR frame")
	}

	// 3. Test keyframe prepend logic
	var videoPayload []byte
	streamer.configMu.RLock()
	cfg := streamer.cachedCodecConfig
	if len(cfg) > 0 && !hasSPS(idrPayload) {
		videoPayload = make([]byte, len(cfg)+len(idrPayload))
		copy(videoPayload, cfg)
		copy(videoPayload[len(cfg):], idrPayload)
	} else {
		videoPayload = idrPayload
	}
	streamer.configMu.RUnlock()

	// 4. Packetize through H264Payloader
	packets := streamer.videoPayloader.Payload(1200, videoPayload)
	if len(packets) < 2 {
		t.Fatalf("Expected at least 2 packets (STAP-A + IDR), got %d", len(packets))
	}

	// First packet MUST be STAP-A (Type 24: 0x78)
	if packets[0][0] != 0x78 {
		t.Fatalf("Expected first packet to be STAP-A (0x78), got 0x%02x", packets[0][0])
	}
	t.Logf("[PASS] First packet is RFC 6184 STAP-A containing SPS+PPS: len=%d, header=0x%02x", len(packets[0]), packets[0][0])

	// Second packet MUST be IDR slice (Type 5: 0x65)
	if (packets[1][0] & 0x1F) != 5 {
		t.Fatalf("Expected second packet to be IDR (NAL 5), got NAL type %d", packets[1][0]&0x1F)
	}
	t.Logf("[PASS] Second packet is IDR NAL unit: len=%d, NAL type=%d", len(packets[1]), packets[1][0]&0x1F)

	// 5. Verify subsequent keyframe ALSO produces STAP-A without losing config
	packets2 := streamer.videoPayloader.Payload(1200, videoPayload)
	if len(packets2) < 2 || packets2[0][0] != 0x78 {
		t.Fatalf("Subsequent keyframe failed to produce STAP-A: len=%d", len(packets2))
	}
	t.Log("[PASS] Subsequent keyframe also reliably generated STAP-A SPS+PPS prefix!")
}
