...
func (s *WebRTCSession) EnqueueVideoPacket(packet *rtp.Packet, isKeyFrame bool) {
	...
	for {
		...
		case s.videoQueue <- packet:
			return
		default:
			// Queue is full: drop oldest packet to eliminate buffer delay (<20ms glass-to-glass)
			select {
			case <-s.videoQueue:
			default:
			}
...
func NewWebRTCSession(ctrl *ControlWriter, iceServersRaw string) (*WebRTCSession, error) {
	...
	session := &WebRTCSession{
		...
		audioTrack: aTrack,
		ctrl:       ctrl,
		videoQueue: make(chan *rtp.Packet, 32),
		audioQueue: make(chan *rtp.Packet, 32),
		videoSeq:   vSeqInit,
		audioSeq:   aSeqInit,
	}
	...
	// Listen for RTCP PLI (Picture Loss Indication) and FIR (Full Intra Request) from browser to request IDR keyframe (throttled to max 1 / 500ms)
	go func() {
		rtcpBuf := make([]byte, 1500)
		for {
			n, _, rtcpErr := vSender.Read(rtcpBuf)
			if rtcpErr != nil {
				return
			}
