...
func (sb *StreamerBridge) SetControlWriter(ctrl *ControlWriter) {
	...
	defer sb.mu.Unlock()
	sb.control = ctrl
	if ctrl != nil && sb.videoWidth > 0 && sb.videoHeight > 0 {
		ctrl.SetVideoSize(int(sb.videoWidth), int(sb.videoHeight))
	}
}

func (sb *StreamerBridge) SetOnVideoClosed(fn func()) {
	sb.mu.Lock()
	defer sb.mu.Unlock()
	sb.onVideoClosed = fn
}

func (sb *StreamerBridge) GetVideoDimensions() (uint32, uint32) {
	sb.mu.Lock()
	defer sb.mu.Unlock()
	return sb.videoWidth, sb.videoHeight
}
...
func (sb *StreamerBridge) StreamVideo(conn net.Conn) {
	...
	sb.mu.Lock()
	sb.videoWidth = width
	sb.videoHeight = height
	if sb.control != nil {
		sb.control.SetVideoSize(int(width), int(height))
	}
	sb.mu.Unlock()
	...
	defer func() {
		...
		sb.mu.Unlock()
		if cb != nil && running {
			log.Printf("[Streamer] Video stream closed; notifying supervisor for auto-restart")
			go cb()
		}
	}()
	...
	for sb.running {
		...
		if isConfig {
			...
			sb.configMu.Unlock()
			log.Printf("[Streamer] Codec Configuration received (SPS/PPS, %d bytes)", len(payload))

			// Deliver config packets directly to any active WebRTC sessions
			configPacketsData := parseH264ConfigPackets(payload)
			if len(configPacketsData) > 0 {
				sb.sessionsMu.RLock()
				cachedTs := sb.timeline.LastVideoRtpTs()
				for _, sess := range sb.sessions {
					for i, pData := range configPacketsData {
		...
		// When a keyframe arrives from scrcpy, ensure SPS and PPS are prepended
		// so decoders (WebRTC and Preview) can immediately initialize and render without waiting.
		var videoPayload []byte
		if isKeyFrame {
			sb.configMu.RLock()
			cfg := sb.cachedCodecConfig
			if len(cfg) > 0 && !hasSPS(payload) {
				videoPayload = make([]byte, len(cfg)+len(payload))
				copy(videoPayload, cfg)
				copy(videoPayload[len(cfg):], payload)
			} else {
		...
		sb.packetizeAndBroadcastVideo(videoPayload, rtpTimestamp, isKeyFrame)

		// WebSocket Fallback Preview Stream (PREV framing)
		if sb.previewStreamer != nil && sb.previewStreamer.IsActive() {
			_ = sb.previewStreamer.SendFrame(videoPayload, isKeyFrame, ptsUs)
		}
	}
}
...
// hasSPS checks if an H.264 Annex B stream contains an SPS (NAL unit type 7)
