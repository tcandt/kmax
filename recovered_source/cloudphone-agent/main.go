...
func main() {
	...
	flag.StringVar(&cfg.IceServers, "ice-servers", cfg.IceServers, "ICE servers (STUN/TURN)")
	flag.Parse()
	...
	streamer.SetOnVideoClosed(func() {
		restartMu.Lock()
		...
		log.Printf("[Agent] Video stream ended; auto-restarting scrcpy helper...")
		if err := scrcpy.Restart(scrcpy.GetCurrentOptions(), streamer); err != nil {
			log.Printf("[Agent] Failed to auto-restart scrcpy: %v", err)
		} else {
			log.Printf("[Agent] Successfully auto-restarted scrcpy helper")
			if cw := scrcpy.GetControlWriter(); cw != nil {
				sessionsMu.RLock()
				for _, sess := range sessions {
					sess.SetControlWriter(cw)
				}
				sessionsMu.RUnlock()
	...
	for {
		...
		rawWS, err := signalingDialer.Dial(targetEndpoint, sniHost)
		if err != nil {
			wait := backoff.NextWait()
		...
		for {
			...
			if msgType == "client_msg" || messageType == "forward" {
				...
				case "request-offer":
					...
					if !scrcpy.IsAlive() {
						log.Printf("[Agent] Scrcpy helper is not alive on request-offer; restarting...")
						if err := scrcpy.Restart(scrcpy.GetCurrentOptions(), streamer); err != nil {
							log.Printf("[Agent] Failed to restart scrcpy on request-offer: %v", err)
						}
					}
					...
					session.pc.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
						if state == webrtc.PeerConnectionStateConnected {
							log.Printf("[Agent] WebRTC PeerConnection Connected for client: %s -> requesting immediate IDR keyframe", clientID)
							if cw := scrcpy.GetControlWriter(); cw != nil {
								_ = cw.RequestKeyframe()
							}
