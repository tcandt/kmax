...
func (cw *ControlWriter) SetVideoSize(w, h int) {
	cw.mu.Lock()
	defer cw.mu.Unlock()
	cw.videoW = w
	cw.videoH = h
...
func (cw *ControlWriter) GetVideoSize() (int, int) {
	cw.mu.Lock()
	defer cw.mu.Unlock()
	return cw.videoW, cw.videoH
}
...
func (cw *ControlWriter) SendTouch(action byte, pointerID int64, x, y, w, h int, pressure float32) error {
	...
	// Automatically rescale coordinates and adjust (w, h) to match scrcpy encoder's videoSize
	// so that scrcpy's PositionMapper never drops touch events due to dimension mismatch.
	if cw.videoW > 0 && cw.videoH > 0 {
		if w > 0 && h > 0 && (w != cw.videoW || h != cw.videoH) {
			x = int(int64(x) * int64(cw.videoW) / int64(w))
			y = int(int64(y) * int64(cw.videoH) / int64(h))
		}
		w = cw.videoW
		h = cw.videoH
	}
