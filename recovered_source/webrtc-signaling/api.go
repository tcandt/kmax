...
func (s *APIServer) filterDevicesForClient(user *User, share *ShareRecord) []*Device {
	...
	} else if user.MaxDevices > 0 {
		// Quota allocated without specific serial assignment: allow up to MaxDevices
		for idx, d := range all {
			if idx >= user.MaxDevices {
				break
			}
			res = append(res, d)
		}
	...
	// Enforce user MaxDevices quota cap if set
	if user.MaxDevices > 0 && len(res) > user.MaxDevices {
		res = res[:user.MaxDevices]
	}
	return res
}
...
func canAccessDevice(user *User, share *ShareRecord, noAuth bool, targetDeviceID string) bool {
	...
	if len(user.AssignedDevices) == 0 && user.MaxDevices > 0 {
		return true
	}
	return false
}
...
func (s *APIServer) handleAdminUserUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if s.store != nil {
		...
		if targetUser != nil {
			...
			} else if req.ExpireSeconds == 0 {
				targetUser.ExpiresAt = ""
			}
			if req.MaxDevices != nil {
				targetUser.MaxDevices = *req.MaxDevices
				s.auth.SetUserDeviceQuota(req.Username, *req.MaxDevices)
			}
...
func (s *APIServer) handleLicenseStatus(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]interface{}{
		...
		"license_source":         "Full Source Enterprise License",
		"machine_id":             "ENTERPRISE-UNLIMITED",
		"max_devices":            maxDevs,
		"post_promo_max_devices": maxDevs,
		"promo":                  false,
		"status":                 "valid",
	})
}
...
func (s *APIServer) handleShareExtend(w http.ResponseWriter, r *http.Request) {
	...
	if share.ExpiresAt.IsZero() {
		...
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
			"code": 400,
			"msg":  "Permanent shares do not require extension",
		})
		return
	}
...
func (s *APIServer) handleAdminUserCreate(w http.ResponseWriter, r *http.Request) {
	...
	createdUser, err := s.auth.RegisterUser(req.Username, req.Password, req.Role, req.Note, req.MaxDevices)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	if req.ExpireSeconds > 0 && createdUser != nil {
		createdUser.ExpiresAt = time.Now().Add(time.Duration(req.ExpireSeconds) * time.Second).Format(time.RFC3339)
		if s.store != nil {
			s.store.SaveUser(createdUser)
		}
	}
	w.Header().Set("Content-Type", "application/json")
	_ = json.NewEncoder(w).Encode(map[string]interface{}{
...
func (s *APIServer) handleAdminAssign(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewDecoder(r.Body).Decode(&req)
	if req.Username != "" {
		if len(req.Devices) > 0 || req.DeviceID == "" {
			s.auth.AssignDevices(req.Username, req.Devices)
