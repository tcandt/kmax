...
func (a *AuthManager) RegisterUser(username, password, role, note string, maxDevices ...int) (*User, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *AuthManager) AssignDevices(username string, devices []string) bool {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *AuthManager) SetUserDeviceQuota(username string, maxDevices int) bool {
	a.mu.Lock()
	defer a.mu.Unlock()
